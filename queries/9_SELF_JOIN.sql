...
-- 4. Give the number of routes that visit either London Road (149) or Craiglockhart (53). The two services that link these stops have a count of 2. Restrict the output to these two routes.
...
FROM (SELECT num, company, stopb.name stopb
  FROM route a JOIN route b ON (a.company = b.company AND a.num = b.num) JOIN stops stopa ON stopa.id = a.stop JOIN stops stopb ON stopb.id = b.stop
